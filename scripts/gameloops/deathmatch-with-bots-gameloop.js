import { snakeArrSize, gameloopInterval, numOfPlayers, playerControls, playerColors, gameContainerStyles, rows, cols, numOfBots } from "../config.js";
...
const numOfPlayersAndBots = numOfPlayers + numOfBots;

//front of js array is filled with players while the rest with bots
let snakes = [];
for (let i = 0; i < numOfPlayersAndBots; i++)
  snakes[i] = new Snake;
...
setInterval(() => {
  ...
  for (let i = 0; i < numOfPlayersAndBots; i++) {
    ...
    for (let j = 0; j < numOfPlayersAndBots; j++) {
      if (i == j)
  ...
  if (numOfPlayersAndBots - deadSnakesQueue.length <= 1) {
